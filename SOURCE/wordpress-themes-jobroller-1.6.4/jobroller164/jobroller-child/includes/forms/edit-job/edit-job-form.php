...
function jr_edit_job_form( $relisting = false ) {
	...
	<form action="<?php echo get_permalink( $post->ID ); ?>?edit=<?php echo $job_details->ID; ?>" method="post" enctype="multipart/form-data" id="submit_form" class="submit_form main_form">
		...
		<fieldset>
			...
			<p class="<?php if (get_option('jr_enable_job_cities_field')!=='yes') : echo 'optional'; endif; ?>"><label for="job_cities"><?php _e('Job Location', 'appthemes'); ?> <?php if (get_option('jr_enable_job_cities_field')=='yes') : ?><span title="required">*</span><?php endif; ?></label> <?php
			
				$sel = 0;
				global $featured_job_cities_id;
				if (isset($posted['job_term_cities']) && $posted['job_term_cities']>0) :
					$sel = $posted['job_term_cities']; 
				else :
					$get_terms = wp_get_post_terms( $job_details->ID, 'job_cities' );
					foreach ($get_terms as $term) :
						if ($term->term_id!==$featured_job_cities_id) $sel = $term->term_id;
					endforeach;
				endif;
				$args = array(
				    'orderby'            => 'name', 
				...
				$dropdown = wp_dropdown_categories( $args );
				$dropdown = str_replace('class=\'job_cities\' >','class=\'job_cities\' ><option value="">Select a location&hellip;</option>',$dropdown);
				echo $dropdown;
			?></p>
